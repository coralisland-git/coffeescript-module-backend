...
(function() {
  var EdgeAppConfig, argv, chalk, encrypter, exreport, fs, jsonfile, ninja, numeral, os, tty, winston,
  ...
  EdgeAppConfig = (function() {
    EdgeAppConfig.prototype.devMode = true;

    EdgeAppConfig.prototype.useSshTunnel = !true;
    ...
    EdgeAppConfig.prototype.SocketHost = "http://" + os.hostname() + ":8001";
    ...
    EdgeAppConfig.prototype.ConfigPath = [__dirname + "/../EdgeConfig/", process.env.HOME + "/EdigeConfig/", __dirname, __dirname + "/node_modules/edgeconfigcommon/EdgeConfig/"];

    EdgeAppConfig.prototype.logPath = __dirname + "/../logs/";

    EdgeAppConfig.prototype.imagePath = __dirname + "/../images/";

    EdgeAppConfig.prototype.importPath = __dirname + "/../import/";

    EdgeAppConfig.FindFileInPath = function(filename, pathList) {
      var e, filenameTest, i, len, path, stat;
    ...
    function EdgeAppConfig() {
      ...
      this.argv = argv;
      if (os.hostname() === "bpollack-d2fd0b" && this.devMode) {
        this.useSshTunnel = true;
      } else {
        this.devMode = false;
    ...
    EdgeAppConfig.prototype.getCredentials = function(serverCode) {
      ...
      if (this.__credentials == null) {
        configFile = EdgeAppConfig.FindFileInPath("key.txt", this.ConfigPath);
        if (configFile == null) {
        ...
        configFile = EdgeAppConfig.FindFileInPath(jsonTextFile, this.ConfigPath);
        if (configFile == null) {
    ...
    EdgeAppConfig.prototype.getLogger = function(name) {
      ...
      consoleLevel = "log";
      if (this.traceEnabled) {
        consoleLevel = "info,log,error";
      }
      if (this.__logs[name] == null) {
        this.__logs[name] = new winston.Logger({
          transports: [
            new winston.transports.Console({
              ...
              name: "info",
              level: "info",
              filename: this.logPath + name + "-info.log",
              json: true,
              timestamp: true,
              ...
              name: "error",
              level: "error",
              filename: this.logPath + name + "-error.log",
              json: true,
              timestamp: true,
              maxsize: 1024 * 1024 * 40,
              maxFiles: 10,
    ...
    EdgeAppConfig.prototype.timerStart = function(name) {
      ...
      data.status = function() {
        var segmentms, status, statusAll, str;
        ...
        if ((statusAll != null) && statusAll.length === 1 && typeof statusAll[0] === "string") {
          ...
          status = status.slice(0, 301).replace("\n", ", ");
        }
